...
import { TiposEdicion } from '../../../enums';
...
class FormAgendar extends React.PureComponent {
    constructor(props) {
        super(props);
        ...
        this.CommandButton = connectProps(CommandButtonComponent, () => {
            const { agendaNuevaSeleccionada } = this.state;
            return {
                mostrarVisualizar: agendaNuevaSeleccionada,
            };
        });

        this.CustomDayTimeTableCell = connectProps(CustomDayTimeTableCellComponent, () => {
            return {
                fechaResaltar: this.props.fechaResaltar,
        ...
        this.appointmentComponent = connectProps(CustomAppointmentComponent, () => {
            return {
                ...
                condicionMarcarEditable: this.esAgendaNueva,
            };
        });
    ...
    ejecutarCambiosAgenda = ({ added, changed, deleted }) => {
        ...
        if (added) {
            ...
            this.props.onAgendaCreada(nuevosDatosAgenda);
        } else if (changed) {
            const { agendaNueva } = this.state;
            nuevosDatosAgenda = { ...agendaNueva, ...changed.undefined };
            this.props.onAgendaModificada(nuevosDatosAgenda, TiposEdicion.MODIFICADO);
        } else {
            this.props.onAgendaModificada(nuevosDatosAgenda, TiposEdicion.ELIMINADO);
        }
        this.setState({ agendaNueva: nuevosDatosAgenda });
    }
    ...
    abrirEditarAgenda = (info) => {
        this.setState({ FormAgendarsoloLectura: !this.esAgendaNueva(info) });
    }

    esAgendaNueva = (datosAgenda) => {
        return datosAgenda?.id === this.state.agendaNueva.id;
    }

    permitirArrastrar = (info) => {
        return this.esAgendaNueva(info);
    }

    permitirRedimensionar = (info) => {
        return this.esAgendaNueva(info);
    }
    ...
    beforeAppointmentClick = (info, clickFn) => {
        const agendaNuevaSeleccionada = this.esAgendaNueva(info.data);
        this.setState({ agendaNuevaSeleccionada }, clickFn(info));
    }
    ...
    render() {
        ...
        const data = checkEmptyObject(agendaNueva) ? agendas : [...agendas, agendaNueva];
                ...
                <Grid item xs={12}>
                    <Paper>
                        ...
                        >
                            ...
                            <EditRecurrenceMenu />
                            <AppointmentTooltip
                                showDeleteButton={agendaNuevaSeleccionada}
                                showOpenButton
                                showCloseButton
                            ...
                            <DragDropProvider allowDrag={this.permitirArrastrar} allowResize={this.permitirRedimensionar} />
    ...
    CommandLayout = (props) => {
        return <AppointmentForm.CommandLayout
            {...props}
            hideDeleteButton={!this.state.agendaNuevaSeleccionada}
        />
    };
}
...
export default connect(null, mapDispatchToProps)(withStyles(FormStyles)(FormAgendar));